...
{ 
	global $context, $txt, $settings, $scripturl, $modSettings, $sourcedir, $db_prefix;
	...
	} elseif (isset($_REQUEST['sa']) && !empty($_REQUEST['sa'])) {
		$context['SMFPredictionLeague']['action'] = $_REQUEST['sa'];
	} else {
		$context['SMFPredictionLeague']['action'] = DEFAULT_ACTION;
	}
	
	// Used for showing admin index
	adminIndex('SMFPredictionLeague_admin');

	// If the week has been posted back, set this as it will be required
	$context['SMFPredictionLeague']['selected_week'] = isset($_POST['week']) ? $_POST['week'] : DEFAULT_WEEK;
	...
	switch ($context['SMFPredictionLeague']['action']) {
		...
		case 'save_results' :
			...
			// Update prediction results now we have entered new results
		...
		case 'increment_week':
			...
			// Need to update the standings so this week is calculated in the table
			updateStandings();
...
function saveUpdates($saveType) {
	...
	if ($saveType == 'results') {
		...
		db_query($teamQuery, __FILE__, __LINE__);
	}
	
	// We only need to update if results/fixtures have been entered
	if (isset($matches) && sizeof($matches) > 0 && $_POST["SendPMs"] == 'on') {
...
function updateStandings() {
	...
	$query = getStandingsToUpdateSql(1);
	...
	$result3 = db_query($query, __FILE__, __LINE__);
	...
	if (isset($_GET['type'])) {
		switch ($_GET['type']) {
			...
			case 'wipeleague':
				...
				// If debug is on and this is the admin output trace information
				if ($modSettings["SMFPredictionLeague_debugOn"] == 'on' && $context['user']['is_admin'] == 1) {
					$context["SMFPredictionLeague_debugOutput"] .= '<font color="blue">getResetWeekSql</font> - <font color="orange">db_query</font> - <font color="green">' . $query . '</font><br/>';
				}

...
function GetTeams() {
	...
	// Build teams context value with results from GetReturnTeamsSql query. Note that we pass variables to this through the $vars array
	$rows = BuildContextPageFromDb('GetReturnTeamsSql', 'teams', null);
...
function JoinLeague() {
	...
	global $context, $smcFunc, $user_info, $modSettings;
	// Update the members table data for the flag determining whether this user is registered for the prediction league
	updateMemberData($user_info['id'], array('pl_enabled' => 1));
	
	$receivePMs = $_POST['ReceivePMNotification'];
	
	$query = GetJoinLeagueSql($receivePMs, $user_info['id']);
